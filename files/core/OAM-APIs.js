...
/**
 * @param {message} options
 * @param {rawPaste} options
 * @returns {message}
 * @returns {rawPaste}
 * @since Development Build 1.13
 * @description Posts to https://hastebin.com
 */
OpenAudioAPI.hastebinPost = function(options) {
    var defaults = {
        message: null,
    ...
    if (actual.message !== null) {
        $.post({
            url: 'http://thegearmc.xyz/server/hastebin.php',
            data: actual.message
            ...
            console.log(result);
                if (!result.key) {
                    swal({
                        type: 'error',
                        title: 'Failed to post to hastebin'
                    })
                } else {
                    swal({
                        type: 'success',
                        title: 'Posted to hastebin',
                        text: "Give this URL to Spigot plugin page or on discord \n" + 'https://hastebin.com/' + result.key,
                        showCancelButton: false
                    })
                }
            });
    } else {
        console.warn('[OpenAudioAPI] [errorException] type ' + actual.message + ' cannot be empty');
    }
};